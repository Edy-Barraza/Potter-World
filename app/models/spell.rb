...
class Spell
  ...
  def mention_count
    Mention.data.count {|x| x["Spell"]==@name }
  end
  ...
  def names_with_same_first_letter
    names =[]
    Spell.data.each {|x| if x["Spell(Lower)"][0,1]==@name[0,1] then names.push(x["Spell(Lower)"]) end}
    return names
  end
  ...
  def self.find_by_mention(mention)
    Spell.data.each {|s| if s["Spell(Lower)"] == mention.name then return Spell.new(s) end}
    return nil
  end
